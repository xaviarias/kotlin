...
object CommonPatterns {
    ...
    val multilineCommentRegex = """\/\*\s+?%s\s+\*\/(?:$ls)*"""
    val directiveRegex =
    ...
    val testInfoElementPattern: Pattern = Pattern.compile("""(?: \* )?(?<name>[A-Z ]+?)(?::\s*(?<value>.*?))?$ls""")
    ...
    val packagePattern: Pattern = Pattern.compile("""(?:^|$ls)package (?<packageName>.*?)(?:;|$ls)""")
}
...
object NotLinkedSpecTestPatterns : BasePatterns {
    ...
    override val testInfoPattern: Pattern =
        Pattern.compile(multilineCommentRegex.format(""" $ASTERISK_REGEX KOTLIN $testAreaRegex NOT LINKED SPEC TEST \($testTypeRegex\)$ls(?<infoElements>[\s\S]*?$ls)"""))
}
...
object LinkedSpecTestPatterns : BasePatterns {
    ...
    override val testPathPattern: Pattern =
        Pattern.compile(testPathRegexTemplate.format(pathPartRegex, FILENAME_REGEX))
    override val testInfoPattern: Pattern =
        Pattern.compile(multilineCommentRegex.format(""" $ASTERISK_REGEX KOTLIN $testAreaRegex SPEC TEST \($testTypeRegex\)$ls(?<infoElements>[\s\S]*?$ls)"""))
...
object TestCasePatterns {
    ...
    private val testCaseInfoElementsRegex = """(?<%s>%s${SpecTestCaseInfoElementType.TESTCASE_NUMBER.name.withSpaces()}:%s*?$ls)"""
